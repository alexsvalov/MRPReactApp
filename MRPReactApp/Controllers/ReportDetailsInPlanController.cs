...
namespace MRPReactApp.Controllers
{
    ...
    [ApiController]
    public class ReportDetailsInPlanController : ControllerBase
    {

        private readonly dbMRPprodContext _context;
        private readonly List<ProductAggregate> result = new();

        public ReportDetailsInPlanController(dbMRPprodContext context)
        {
            _context = context;
        }

        // GET api/<GetProductsInPlanController>/5
        [HttpGet("{id}")]
        public List<ProductAggregate>? GetDetailsInPlan(int id)
        ...
        public List<ProductAggregate> GetComponents(List<ProductAggregate> products)
        {
            ...
            result.AddRange(products.Where(p => p.Purchase != true && p.Type == 1));
            selected.AddRange(products.Where(p => p.Purchase != true && p.Type != 1));

            foreach (ProductAggregate selectedItem in selected)
            {
                List<ProductAggregate> components =
                    (from specification in _context.Specification
                     ...
                     where selectedItem.Id == specification.Product
                     select new ProductAggregate
                     {
                         Id = component.Id,
                         Code = component.Code,
                         ...
                         Quantity = selectedItem.Quantity * specification.Quantity
                     }).ToList();
            ...
            {
                result.AddRange(selectedComponents.Where(p => p.Type == 1 && p.Purchase != true));
                resultTemp.AddRange(selectedComponents.Where(p => p.Type != 1 && p.Purchase != true));
